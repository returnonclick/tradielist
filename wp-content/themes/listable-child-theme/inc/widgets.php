...
/* ===== REWRITING Theme plugins to change something ===================== */

// add_action( 'widgets_init', 'wp_roc_parent_unregister_widgets', 10 );
 
// function wp_roc_parent_unregister_widgets() {
//     // remove Parent registered Widget
//     //unregister_widget( 'Front_Page_Listing_Categories_Widget' ); 
//     // register a custom Widget (if needed)
// }

//	ESPECIAL NOTE: I have tryied to remove the original plugin but unregister_widget function was not working, so I moved on to do not waste time and changed the widget name and ID   	

register_widget( 'ROCFront_Page_Listing_Categories_Widget' );

class ROCFront_Page_Listing_Categories_Widget extends WP_Widget {
...
//		'default_image'   => ''
	...
	function __construct() {
		parent::__construct(
			'roc_front_page_listing_categories', // Base ID
			'&#x1f535; ' . esc_html__( 'Front Page', 'listable' ) . ' &raquo; ' . esc_html__( 'ROC Listing Categories', 'listable' ), // Name
			array( 'description' => esc_html__( 'Display a list of listing categories based on different criteria (eg. most popular, random) or specify which ones you want to show. By ROC', 'listable' ), ) // Args
		);
	}
							...
							<div class="category-cover" style="background-image: url(<?php echo listable_get_inline_background_image( $image_src ); ?>)">
								<a href="<?php echo esc_url( get_term_link( $term ) ); ?>">
									...
									<?php if ( ! empty( $icon_url ) ) : ?>
										...
										<div class="category-icon-disabled">
...
} // class ROCFront_Page_Listing_Categories_Widget
...
register_widget( 'ROCFront_Page_Listing_Cards_Widget' );

class ROCFront_Page_Listing_Cards_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'roc_front_page_listing_cards', // Base ID
			'&#x1f535; ' . esc_html__( 'Front Page', 'listable' ) . ' &raquo; ' . esc_html__( 'ROCListing Cards', 'listable' ), // Name
			array( 'description' => esc_html__( 'Displays a list of your listings based on different criteria (eg. latest of featured listings from a specific category). By ROC', 'listable' ), ) // Args
		);
	}
...
class Business_Contact extends WP_Widget {
    ...
    public function widget( $args, $instance ) {
		...
		if ( $facebook = get_post_meta( get_the_ID(), '_company_facebook', true) ) {
			$facebook_pure = preg_replace('#^https?://#', '', rtrim(esc_url($facebook),'/'));
		}

		$abn = get_post_meta( get_the_ID(), '_business_abn', true);
		$acn = get_post_meta( get_the_ID(), '_business_acn', true);
		$license1 = get_post_meta( get_the_ID(), '_business_license1', true);
		$license2 = get_post_meta( get_the_ID(), '_business_license2', true);
		$license3 = get_post_meta( get_the_ID(), '_business_license3', true);
